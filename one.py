...
class OneNote:
    ...
    def save_one_note_info(self, url, need_cover=False, info='', dir_path='datas',keyword='keyword'):
        try:
            ...
            if title.strip() == '':
                title = f'无标题'
            # path = f'./{dir_path}/{keyword}/{nickname}_{user_id}/{title}_{note.note_id}'
            ...
            # 数据库连接
            # conn = connect(host='localhost',port=3306,user='root',password='123456',database='spider',charset='utf8')
            # print(conn)
            # 创建Cursor对象
            # cs1 = conn.cursor() 
            if note_type == 'normal':
                ...
                # 使用tqdm显示进度条
                for img_index, img in enumerate(tqdm(note.image_list, desc=f"用户-{nickname}资源下载进度")):
                    img_url = img['info_list'][1]['url']
                    # 存入到数据库表spider中（id、name、userId、content、contentId）
                    # 执行DML语句插入数据
                    # cs1.execute('insert into content(name,userId,content,contentId) values(%s,%s,%s,%s)',[nickname,user_id,title,note.note_id])
                    download_media(path, f'image_{img_index}', img_url, 'image', f'第{img_index}张图片')
    ...
    def main(self, url_list):
        ...
        # 使用tqdm显示进度条
        for url in tqdm(url_list, desc='笔记下载进度'):
            try:
                self.save_one_note_info(url)
            except:
                print(f'笔记 {url} 保存失败')
...
if __name__ == '__main__':
    one_note = OneNote()
    url_list = [
        'https://www.xiaohongshu.com/explore/5d93428c0000000001005da8',
        # 'https://www.xiaohongshu.com/explore/63d625f8000000001d01042c',
        # 'https://www.xiaohongshu.com/explore/61ac8820000000002103a8aa',
        # 'https://www.xiaohongshu.com/explore/62d2699c000000001200f101',
        # 'https://www.xiaohongshu.com/explore/637f0938000000001f012d15',
