    ...
    for i in range(5):
        try:
            if type == 'image':
                ...
                # with open(path + '/' + name + '.jpg', mode="wb") as f:
                #     for data in tqdm(res.iter_content(chunk_size=chunk_size), total=content_size // chunk_size, unit='MB', desc=f"{info}图片下载进度"):
                #         f.write(data)
                #         size += len(data)
                    # print(f"{info}图片下载完成")
                ...
                with open(path + '/' + name + '.mp4', mode="wb") as f:
                    ...
                    # print(f"{name}下载完成")
                    # 使用tqdm显示进度条
                    for data in tqdm(res.iter_content(chunk_size=chunk_size), total=content_size // chunk_size, unit='MB', desc=f"{name}下载进度"):
                        f.write(data)
                        size += len(data)
                    # print(f"{name}下载完成")
            break
